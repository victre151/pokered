Route16Gate1F_Script:
	ld hl, wStatusFlags6
	res BIT_ALWAYS_ON_BIKE, [hl]
	call EnableAutoTextBoxDrawing
	ld a, [wRoute16Gate1FCurScript]
	ld hl, Route16Gate1F_ScriptPointers
	jp CallFunctionInTable

Route16Gate1F_ScriptPointers:
	def_script_pointers
	dw_const Route16Gate1FDefaultScript,           				SCRIPT_ROUTE16GATE1F_DEFAULT
	dw_const Route16Gate1FPlayerMovingUpScript,    				SCRIPT_ROUTE16GATE1F_PLAYER_MOVING_UP
	dw_const Route16Gate1FGuardScript,             				SCRIPT_ROUTE16GATE1F_GUARD
	dw_const Route16Gate1FPlayerMovingRightScript, 				SCRIPT_ROUTE16GATE1F_PLAYER_MOVING_RIGHT
	dw_const Route16Gate1FPlayerMovingAfterCyclingClosedScript, SCRIPT_ROUTE16GATE1F_PLAYER_MOVING_AFTER_CYCLING_CLOSED

Route16Gate1FDefaultScript:
	ld hl, .StopsPlayerCoords
	call ArePlayerCoordsInArray
	ret nc
	
	ld hl, wObtainedBadges
	bit BIT_SOULBADGE, [hl]
	jr z, .no_soul_badge
	
	call Route16Gate1FIsBicycleInBagScript
	ret nz
	
	call UpdateSprites
	ld a, TEXT_ROUTE16GATE1F_GUARD_WAIT_UP
	ldh [hTextID], a
	call DisplayTextID
	xor a
	ldh [hJoyHeld], a
	ld a, [wCoordIndex]
	cp $1
	jr z, .next_to_counter
	ld a, [wCoordIndex]
	dec a
	ld [wSimulatedJoypadStatesIndex], a
	ld b, $0
	ld c, a
	ld a, PAD_UP
	ld hl, wSimulatedJoypadStatesEnd
	call FillMemory
	call StartSimulatingJoypadStates
	ld a, SCRIPT_ROUTE16GATE1F_PLAYER_MOVING_UP
	ld [wRoute16Gate1FCurScript], a
	ret
	
.no_soul_badge
	call Route16Gate1FIsBicycleInBagScript
	jr z, .no_soul_no_bike
	
	call UpdateSprites
	ld a, TEXT_ROUTE16GATE1F_GUARD_CYCLING_ROAD_CLOSED
	ldh [hTextID], a
	call DisplayTextID
	ld a, PAD_BUTTONS | PAD_CTRL_PAD
	ld [wJoyIgnore], a
	xor a
	ldh [hJoyHeld], a
	ld a, PAD_RIGHT
	ld c, 1
	call Route16Gate1FEntranceAutoWalk
	ld a, PAD_CTRL_PAD
	ld [wJoyIgnore], a
	ld a, SCRIPT_ROUTE16GATE1F_PLAYER_MOVING_AFTER_CYCLING_CLOSED
	ld [wRoute16Gate1FCurScript], a
	ret

.no_soul_no_bike
	call UpdateSprites
	ld a, TEXT_ROUTE16GATE1F_GUARD
	ldh [hTextID], a
	call DisplayTextID
	xor a
	ld [wJoyIgnore], a
	ld a, SCRIPT_ROUTE16GATE1F_DEFAULT
	ld [wRoute16Gate1FCurScript], a
	ret

.next_to_counter
	ld a, SCRIPT_ROUTE16GATE1F_GUARD
	ld [wRoute16Gate1FCurScript], a
	ret
	
.StopsPlayerCoords:
	dbmapcoord  4,  7
	dbmapcoord  4,  8
	dbmapcoord  4,  9
	dbmapcoord  4, 10
	db -1 ; end
	
Route16Gate1FPlayerMovingUpScript:
	ld a, [wSimulatedJoypadStatesIndex]
	and a
	ret nz
	ld a, PAD_CTRL_PAD
	ld [wJoyIgnore], a

Route16Gate1FGuardScript:
	call UpdateSprites
	ld a, TEXT_ROUTE16GATE1F_GUARD
	ldh [hTextID], a
	call DisplayTextID
	call Route16Gate1FIsBicycleInBagScript
	jr z, .no_bike_after_guard
	ld hl, wObtainedBadges
	bit BIT_SOULBADGE, [hl]
	jr z, .no_soul_badge_has_bike
	ld a, $1
	ld [wSimulatedJoypadStatesIndex], a
	ld a, PAD_RIGHT
	ld [wSimulatedJoypadStatesEnd], a
	call StartSimulatingJoypadStates
	ld a, SCRIPT_ROUTE16GATE1F_PLAYER_MOVING_RIGHT
	ld [wRoute16Gate1FCurScript], a
	ret
	
.no_soul_badge_has_bike
	ld a, PAD_BUTTONS | PAD_CTRL_PAD
	ld [wJoyIgnore], a
	xor a
	ldh [hJoyHeld], a
	ld a, PAD_RIGHT
	ld c, 1
	call Route16Gate1FEntranceAutoWalk
	ld a, PAD_CTRL_PAD
	ld [wJoyIgnore], a
	ld a, SCRIPT_ROUTE16GATE1F_PLAYER_MOVING_AFTER_CYCLING_CLOSED
	ld [wRoute16Gate1FCurScript], a
	ret	
	
.no_bike_after_guard
	ld hl, wObtainedBadges
	bit BIT_SOULBADGE, [hl]
	jr z, .no_bike_no_soul_done
	ld a, PAD_BUTTONS | PAD_CTRL_PAD
	ld [wJoyIgnore], a
	xor a
	ldh [hJoyHeld], a
	ld a, PAD_RIGHT
	ld c, 1
	call Route16Gate1FEntranceAutoWalk
	ld a, PAD_CTRL_PAD
	ld [wJoyIgnore], a
	ld a, SCRIPT_ROUTE16GATE1F_PLAYER_MOVING_AFTER_CYCLING_CLOSED
	ld [wRoute16Gate1FCurScript], a
	ret
	
.no_bike_no_soul_done
	xor a
	ld [wJoyIgnore], a
	ld a, SCRIPT_ROUTE16GATE1F_DEFAULT
	ld [wRoute16Gate1FCurScript], a
	ret

Route16Gate1FPlayerMovingRightScript:
	ld a, [wSimulatedJoypadStatesIndex]
	and a
	ret nz
	xor a
	ld [wJoyIgnore], a
	ld hl, wStatusFlags5
	res BIT_SCRIPTED_MOVEMENT_STATE, [hl]
	ld a, SCRIPT_ROUTE16GATE1F_DEFAULT
	ld [wRoute16Gate1FCurScript], a
	ret

Route16Gate1FPlayerMovingAfterCyclingClosedScript:
	ld a, [wSimulatedJoypadStatesIndex]
	and a
	ret nz
	xor a
	ld [wJoyIgnore], a
	ld hl, wStatusFlags5
	res BIT_SCRIPTED_MOVEMENT_STATE, [hl]
	ld a, SCRIPT_ROUTE16GATE1F_DEFAULT
	ld [wRoute16Gate1FCurScript], a
	ret

Route16Gate1FEntranceAutoWalk:
	push af
	ld b, 0
	ld a, c
	ld [wSimulatedJoypadStatesIndex], a
	ld hl, wSimulatedJoypadStatesEnd
	pop af
	call FillMemory
	jp StartSimulatingJoypadStates

Route16Gate1FIsBicycleInBagScript:
	ld b, BICYCLE
	jp IsItemInBag

Route16Gate1F_TextPointers:
	def_text_pointers
	dw_const Route16Gate1FGuardText,                  TEXT_ROUTE16GATE1F_GUARD
	dw_const Route16Gate1FGamblerText,                TEXT_ROUTE16GATE1F_GAMBLER
	dw_const Route16Gate1FGuardWaitUpText,            TEXT_ROUTE16GATE1F_GUARD_WAIT_UP
	dw_const Route16Gate1FGuardCyclingRoadClosedText, TEXT_ROUTE16GATE1F_GUARD_CYCLING_ROAD_CLOSED

Route16Gate1FGuardText:
	text_asm
	ld hl, wObtainedBadges
	bit BIT_SOULBADGE, [hl]
	jr z, .no_soul_badge
	
	call Route16Gate1FIsBicycleInBagScript
	jr z, .soul_no_bike
	ld hl, .CyclingRoadExplanationText
	call PrintText
	jp TextScriptEnd

.no_soul_badge
	call Route16Gate1FIsBicycleInBagScript
	jr z, .no_soul_no_bike
	ld hl, Route16Gate1FGuardCyclingRoadClosedText
	call PrintText
	jp TextScriptEnd
	
.no_soul_no_bike
	ld hl, .NoPedestriansAllowedText
	call PrintText
	jp TextScriptEnd
	
.soul_no_bike
	ld hl, .NoPedestriansAllowedText
	call PrintText
	jp TextScriptEnd

	
.NoPedestriansAllowedText:
	text_far _Route16Gate1FGuardNoPedestriansAllowedText
	text_end
	
.CyclingRoadExplanationText:
	text_far _Route16Gate1FGuardCyclingRoadExplanationText
	text_end
	
Route16Gate1FGuardCyclingRoadClosedText:
	text_far _Route16Gate1FGuardCyclingRoadClosedText
	text_end
	
Route16Gate1FGuardWaitUpText:
	text_far _Route16Gate1FGuardWaitUpText
	text_end
	
Route16Gate1FGamblerText:
	text_far _Route16Gate1FGamblerText
	text_end